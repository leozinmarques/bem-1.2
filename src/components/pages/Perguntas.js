...
import {Button, FormControl, FormGroup, Modal, Table, Grid, Col, Row, Form} from 'react-bootstrap';
  ...
  constructor(props) {
    ...
    this.question = (evento) => {
      ...
      if (evento.target.getAttribute('tipo') === "delete"){
        var alternativasAux = this.state.alternativas;
        alternativasAux.splice(evento.target.getAttribute('id'),1);
        this.setState({
          alternativas: alternativasAux
        });
      }
    }
  }
